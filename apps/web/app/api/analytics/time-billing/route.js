...
export async function GET(request) {
  try {
    ...
    if (startDate && endDate) {
      dateFilter = sql`AND te.started_at BETWEEN ${startDate} AND ${endDate}`;
    } else if (startDate) {
      dateFilter = sql`AND te.started_at >= ${startDate}`;
    } else if (endDate) {
      dateFilter = sql`AND te.started_at <= ${endDate}`;
    }
    ...
    const [totals] = await sql`
      ...
      WHERE te.professional_id = ${professionalId} ${dateFilter}
    `;

    // Hours by client
    const clientBreakdown = await sql`
      SELECT 
        c.id,
        c.full_name,
    ...
    const [billableBreakdown] = await sql`
      SELECT 
